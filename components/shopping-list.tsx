...
function ShoppingListComponent({ shoppingList, onToggleItem, onDeleteList }: ShoppingListProps) {
  ...
  const exportList = () => {
    ...
    categories.forEach((category) => {
      ...
      groupedItems[category].forEach((item) => {
        ...
        text += `${checkmark} ${item.quantity} ${item.measure} ${item.food}\n`
      })
      text += "\n"
    })
    ...
    a.href = url
    a.download = `${shoppingList.name.replace(/\s+/g, "_")}.txt`
    a.click()
